...
def _jet_colormap(values: Float[np.ndarray, "n"]) -> Float[np.ndarray, "n 3"]:
    """Apply jet colormap to normalized values in [0, 1]."""
    # Jet colormap: blue -> cyan -> green -> yellow -> red
    r = np.clip(4 * values - 1.5, 0, 1)
    g = np.clip(np.where(values < 0.5, 4 * values, -4 * values + 4.5), 0, 1)
    b = np.clip(-4 * values + 1.5, 0, 1)
    return np.stack([r, g, b], axis=-1)


def _turbo_colormap(values: Float[np.ndarray, "n"]) -> Float[np.ndarray, "n 3"]:
    """Apply turbo colormap to normalized values in [0, 1] using lookup table interpolation."""
    # Clamp values to [0, 1]
    x = np.clip(values, 0.0, 1.0)

    # Map to indices in the 256-entry lookup table
    indices = x * 255.0
    a = indices.astype(np.int32)
    b = np.minimum(255, a + 1)
    f = (indices - a).reshape(-1, 1)  # Fractional part for interpolation

    # Linear interpolation between adjacent colors
    return _TURBO_COLORMAP_DATA[a] + (_TURBO_COLORMAP_DATA[b] - _TURBO_COLORMAP_DATA[a]) * f


def get_heatmap(
    values: Float[np.ndarray, "n"], cmap_name: str = "turbo", invert: bool = False
    ...
    From: https://github.com/f3rm/f3rm/blob/main/f3rm_robot/utils.py#L15

    Args:
        values: Values to convert to a heatmap.
        cmap_name: Name of the colormap to use (supported: 'jet', 'viridis', 'turbo').
    ...
    if cmap_name == "jet":
        rgb = _jet_colormap(values)
    elif cmap_name == "turbo":
        rgb = _turbo_colormap(values)
    else:
        raise ValueError(f"Unsupported colormap: {cmap_name}. Supported: 'jet', 'turbo'")

    return rgb


@lru_cache(maxsize=1)
def get_gripper_mesh(include_sphere: bool = True, radius: float = 0.001) -> o3d.geometry.TriangleMesh:
    """Get a skeleton gripper mesh. From https://github.com/f3rm/f3rm/blob/main/f3rm_robot/utils.py#L95"""
